...
typedef struct tpdf_s {
    ...
    int mask;           // 32bit mask to remove bits before storing in DAC
    ...
    int factor;         // magic number to multiply tpdfvalue and get a 64bit msb at the right place (for xs2 optimization only)
} tpdf_t;

tpdf_t dspTpdfGlobal;   // global instance of the above structure, used by default in each core
...
static inline int dspTpdfPrepare(tpdf_t * current, tpdf_t * local, int dith){
    ...
    if (dith != current->dither){
        ...
        if (bits >= 0) {
            local->factor = (1ULL<<bits)-1;        // we can use a 32x32=64 mul for getting the tpdf scaled directly from the multiplication
...
static inline void dspTpdfInit(int seed, int defaultDither){
    ...
    dspTpdfGlobal.dither = -1; // this will force the below function to compute the global table
    dspTpdfPrepare(&dspTpdfGlobal,&dspTpdfGlobal,0);
...
static inline void dspTpdfApply(dspALU_t *alu, tpdf_t * p){
    ...
    dspmacs64_32_32(alu, tpdf, p->factor);  // this cost less than differentiating both cases (shift or factor)
#else
