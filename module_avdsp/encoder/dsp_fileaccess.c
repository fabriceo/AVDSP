...
int dspCreateIntFile(char * name, int * buff, int size, int dataSize, int numCores, char * begin, char * end){
    dspFileName = name;
...
int dspReadBuffer(char * name, int * buff, int size){
    ...
    printf("file %s opened in read binary , size = %ld bytes\n",name, dspFileSize);
    if (dspFileSize > (size*sizeof(int))) {
        printf("file bigger than predefined buffer in memory\n");
        return -1; }
